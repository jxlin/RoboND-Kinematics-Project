...
namespace tools
{
    void savePathToFile( const std::string fname,
                         const std::vector< geometry_msgs::Pose>& poses )
    {
        ROS_INFO( "Writing to file %s", fname.c_str() );

        std::ofstream _fHandle( fname.c_str() );
        if ( !_fHandle.is_open() )
        {
            ROS_WARN( "Error opening log file %s", fname.c_str() );
            return;
        }

        for ( int q = 0; q < poses.size(); q++ )
        {
            _saveSinglePoseToFile( _fHandle,
                                   poses[q] );
        }

        _fHandle.close();

        ROS_INFO( "Finished writing to file %s", fname.c_str() );
    }

    void _saveSinglePoseToFile( std::ofstream& fHandle, 
                                const geometry_msgs::Pose& pose )
    {
        // Write position
        fHandle << pose.position.x << ";"
                << pose.position.y << ";"
                << pose.position.z << ";";

        // Write orientation
        fHandle << pose.orientation.x << ";"
                << pose.orientation.y << ";"
                << pose.orientation.z << ";"
    ...
    eef_group(GRIPPER_GROUP)
{
    ...
    while (ros::ok())
    {
        ...
        {
            {
                //// Build path so that we save the poses that the IK test will send us
                ...
                //// Save path to file
                tools::savePathToFile( "f_path_to_target.txt", path );
            }
        ...
        {
            {
                ...
                //// Save path to file
                tools::savePathToFile( "f_path_to_dropoff.txt", path );
            }
